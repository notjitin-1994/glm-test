...
//#region src/hooks/useActiveArtifact.d.ts
/**
...
type UseActiveArtifactReturn = {
  /** Whether any artifact is currently active (panel is open). */isArtifactActive: boolean; /** The ID of the currently active artifact, or `null` if none. */
  activeArtifactId: string | null; /** Closes whichever artifact is currently active. No-op if none is active. */
  closeArtifact: () => void;
};
/**
 ...
 */
declare function useActiveArtifact(): UseActiveArtifactReturn;
//#endregion
//#region src/hooks/useArtifact.d.ts
/**
...
type UseArtifactReturn = {
  /** Whether this artifact is the currently active (visible) one. */isActive: boolean; /** Activates this artifact. */
  open: () => void; /** Deactivates this artifact. */
  close: () => void; /** Toggles this artifact: opens if closed, closes if open. */
  toggle: () => void;
};
/**
 ...
 * ```
 */
declare function useArtifact(artifactId: string): UseArtifactReturn;
//#endregion
//#region src/hooks/useArtifactPortalTarget.d.ts
/**
 ...
 * ```
 */
declare function useArtifactPortalTarget(): {
...
type ChatStore = ThreadListState & ThreadListActions & ThreadState & ThreadActions & {
  /** @internal */_nextCursor?: any; /** @internal */
...
type ThreadApiConfig = {
  ...
  threadApiUrl?: never;
  fetchThreadList?: (cursor?: any) => Promise<{
    threads: Thread[];
    nextCursor?: any;
  }>;
  createThread?: (firstMessage: UserMessage) => Promise<Thread>;
...
type ChatProviderProps = ThreadApiConfig & ChatApiConfig & {
...
//#endregion
//#region src/store/artifactTypes.d.ts
/**
 ...
 */
type ArtifactState = {
  /** The currently displayed artifact, or `null` if the panel is collapsed. */activeArtifactId: string | null;
};
/**
...
type ArtifactActions = {
  /** Activates an artifact by ID. */openArtifact: (id: string) => void;
  /**
...
type ArtifactInternals = {
  /** @internal */_artifactPanelNode: HTMLElement | null; /** @internal */
  _setArtifactPanelNode: (node: HTMLElement | null) => void;
};
/** Combined artifact store type (state + actions + internals). */
type ArtifactStore = ArtifactState & ArtifactActions & ArtifactInternals;
//#endregion
//#region src/store/ArtifactContext.d.ts
/** @internal React context holding the artifact Zustand store. Provided by `ChatProvider`. */
declare const ArtifactContext: _$react.Context<StoreApi<ArtifactStore> | null>;
/**
 ...
 */
declare const useArtifactStore: () => StoreApi<ArtifactStore>;
//#endregion
...
//#endregion
export { type AGUIEvent, type ActivityMessage, type ArtifactActions, ArtifactContext, type ArtifactState, type AssistantMessage, type BinaryInputContent, ChatProvider, type ChatProviderProps, type ChatStore, type CreateMessage, type DeveloperMessage, EventType, type FunctionCall, type InputContent, type LangGraphAdapterOptions, type LangChainMessage as LangGraphMessageFormat, type Message, MessageContext, type MessageFormat, MessageProvider, type ReasoningMessage, type StreamProtocolAdapter, type SystemMessage, type TextInputContent, type Thread, type ThreadActions, type ThreadListActions, type ThreadListState, type ThreadState, type ToolCall, type ToolMessage, type UserMessage, agUIAdapter, identityMessageFormat, langGraphAdapter, langGraphMessageFormat, openAIAdapter, openAIConversationMessageFormat, openAIMessageFormat, openAIReadableStreamAdapter, openAIResponsesAdapter, processStreamedMessage, useActiveArtifact, useArtifact, useArtifactPortalTarget, useArtifactStore, useMessage, useThread, useThreadList };
