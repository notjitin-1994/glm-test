...
import { Tabs as OpenUITabs, TabsContent as OpenUITabsContent, TabsList as OpenUITabsList, TabsTrigger as OpenUITabsTrigger, } from "../../components/Tabs";
...
export const Tabs = defineComponent({
    ...
    component: ({ props, renderNode }) => {
        ...
        if (!items.length)
            return null;
        return (_jsxs(OpenUITabs, { value: activeTab, onValueChange: handleValueChange, children: [_jsx(OpenUITabsList, { children: items.map((item) => (_jsx(OpenUITabsTrigger, { value: item.props.value, text: item.props.trigger }, item.props.value))) }), items.map((item) => (_jsx(OpenUITabsContent, { value: item.props.value, children: renderNode(item.props.content) }, item.props.value)))] }));
    },
});
