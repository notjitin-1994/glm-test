...
import { defineComponent, parseStructuredRules, reactive, useFormValidation, useIsStreaming, useStateField, } from "@openuidev/react-lang";
...
export const RadioGroup = defineComponent({
    ...
    component: ({ props }) => {
        ...
        if (!items.length)
            return null;
        return (_jsx(OpenUIRadioGroup, { name: field.name, value: value ?? "", onValueChange: (val) => {
                field.setValue(val);
                if (hasRules) {
                    formValidation?.validateField(field.name, val, rules);
                }
            }, disabled: isStreaming, children: items.map((item, i) => (_jsx(OpenUIRadioItem, { value: item.props.value, label: item.props.label, description: item.props.description || "" }, i))) }));
    },
});
