...
import { useAutoAngleCalculation, useExportChartData, useMaxLabelWidth, useTransformedKeys, useYAxisLabelWidth, } from "../hooks";
import { cartesianGrid, CustomTooltipContent, DefaultLegend, LineInBarShape, SideBarTooltip, SVGXAxisTick, YAxisTick, } from "../shared";
...
const BarChartCondensedComponent = ({ data, categoryKey, theme = "ocean", customPalette, variant = "grouped", tickVariant = "singleLine", grid = true, icons = {}, radius, isAnimationActive = false, showYAxis = true, xAxisLabel, yAxisLabel, legend = true, className, height = CHART_HEIGHT, width, maxBarWidth = DEFAULT_MAX_BAR_WIDTH, }) => {
    ...
    const { angle: calculatedAngle, height: xAxisHeight } = useAutoAngleCalculation(maxLabelWidth, tickVariant === "angled", maxLabelWidth < 100 ? widthOfData : undefined);
    ...
    const yAxis = useMemo(() => {
        ...
        return (_jsx("div", { className: "openui-bar-chart-condensed-y-axis-container", children: _jsxs(RechartsBarChart, { width: yAxisWidth, height: effectiveHeight, data: data, stackOffset: "sign", margin: {
                    top: chartMargin.top,
                    ...
                    right: 0,
                        ...
                        return (_jsx(Bar, { dataKey: key, fill: "transparent", stackId: variant === "stacked" ? "a" : undefined, isAnimationActive: false, maxBarSize: 0 }, `yaxis-bar-chart-condensed-${key}`));
    ...
    const barElements = useMemo(() => {
        return dataKeys.map((key) => {
            ...
            return (_jsx(Bar, { dataKey: key, fill: color, stackId: variant === "stacked" ? "a" : undefined, isAnimationActive: isAnimationActive, maxBarSize: calculatedBarWidth, barSize: calculatedBarWidth, shape: (props) => {
    ...
    return (_jsx(LabelTooltipProvider, { children: _jsx(SideBarTooltipProvider, { isSideBarTooltipOpen: isSideBarTooltipOpen, setIsSideBarTooltipOpen: setIsSideBarTooltipOpen, data: sideBarTooltipData, setData: setSideBarTooltipData, children: _jsxs("div", { className: clsx("openui-bar-chart-condensed-container", className), "data-openui-chart": exportData, style: {
                    width: width ? `${width}px` : undefined,
                }, children: [yAxisLabel && (_jsx("div", { className: "openui-bar-chart-condensed-y-axis-label", children: yAxisLabel })), _jsxs("div", { className: "openui-bar-chart-condensed-container-inner", ref: containerRef, children: [yAxis, _jsx("div", { className: "openui-bar-chart-condensed", ref: chartContainerRef, children: _jsx(ChartContainer, { config: chartConfig, style: {
                                        width: explicitChartWidth ? `${explicitChartWidth}px` : "100%",
                                        height: effectiveHeight,
                                    }, rechartsProps: {
                                        width: explicitChartWidth ?? "100%",
                                        height: effectiveHeight,
                                    }, children: _jsxs(RechartsBarChart, { stackOffset: "sign", accessibilityLayer: true, data: data, margin: chartMargin, barGap: BAR_GAP, barCategoryGap: BAR_CATEGORY_GAP, onMouseMove: handleChartMouseMove, onMouseLeave: handleChartMouseLeave, onClick: onBarClick, width: explicitChartWidth, height: effectiveHeight, children: [grid && cartesianGrid(), _jsx(XAxis, { dataKey: categoryKey, tickLine: false, axisLine: false, textAnchor: isAngled ? "end" : "middle", interval: "preserveStartEnd", minTickGap: 5, height: xAxisHeight, tick: _jsx(SVGXAxisTick, {}), angle: calculatedAngle, orientation: "bottom", padding: {
                                                    left: X_AXIS_PADDING,
                                                    right: X_AXIS_PADDING,
                                                } }), _jsx(ChartTooltip, { cursor: {
                                                    fill: "var(--openui-highlight)",
                                                    stroke: "var(--openui-stroke-default)",
                                                    opacity: 1,
                                                    strokeWidth: 1,
                                                }, content: _jsx(CustomTooltipContent, { parentRef: containerRef }), offset: 10 }), barElements] }, `bar-chart-condensed-${id}`) }) }), isSideBarTooltipOpen && _jsx(SideBarTooltip, { height: effectiveHeight })] }), xAxisLabel && (_jsx("div", { className: "openui-bar-chart-condensed-x-axis-label", children: xAxisLabel })), legend && (_jsx(DefaultLegend, { items: legendItems, containerWidth: effectiveWidth, isExpanded: isLegendExpanded, setIsExpanded: setIsLegendExpanded }))] }) }) }));
};
