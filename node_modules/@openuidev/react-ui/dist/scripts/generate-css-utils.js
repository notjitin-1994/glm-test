...
lines.push("");
const entries = Object.entries(defaultLightTheme).filter(([, value]) => typeof value === "string");
...
lines.push("");
const outputPath = path.resolve(__dirname, "../cssUtils.css");
...
const defaultsLines = [];
defaultsLines.push("// Auto-generated by src/scripts/generate-css-utils.ts — do not edit manually.");
defaultsLines.push("");
defaultsLines.push(":root {");
for (const [key, value] of entries) {
    if (sectionHeaders[key]) {
        if (defaultsLines.length > 3)
            defaultsLines.push("");
...
const darkEntries = Object.entries(defaultDarkTheme).filter(([, value]) => typeof value === "string");
